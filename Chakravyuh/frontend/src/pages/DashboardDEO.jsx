...
const CONTRACTORS = [
  { id: 'Suresh Kumar', name: 'Suresh Kumar', specialty: 'plumbing' },
  { id: 'Ramesh Patel', name: 'Ramesh Patel', specialty: 'electrical' },
  { id: 'Rajendra Singh', name: 'Rajendra Singh', specialty: 'structural' }
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-3xl rounded-xl bg-gray-900 text-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-700 px-6 py-4">
          <h3 className="text-xl font-bold">Review Alert & Assign Contractor</h3>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              {alertItem.photo_url ? (
                <img src={alertItem.photo_url} alt="Submission" className="w-full h-64 object-cover rounded-lg border border-gray-600 bg-gray-800" />
              ) : (
                <div className="w-full h-64 flex items-center justify-center rounded-lg border border-gray-600 bg-gray-800 text-gray-500">
                  No Photo Available
                </div>
              )}
            </div>
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold text-white">{alertItem.school_id}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-gray-400">Category</p>
                  <p className="text-base font-semibold text-white">{toTitle(alertItem.category || '')}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Condition</p>
                  <p className="text-base font-semibold text-white">{alertItem.condition || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Priority Level</p>
                  <span className="rounded-full bg-gray-700 px-2 py-1 text-xs font-semibold">
                    {alertItem.priority_level || 'Medium'}
                  </span>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Risk Score</p>
                  <p className="text-base font-semibold text-white">{alertItem.risk_score ? alertItem.risk_score.toFixed(2) : 'N/A'}</p>
                </div>
              </div>
              <div>
                <p className="text-sm text-gray-400">Submitted By</p>
                <p className="text-lg font-semibold text-white">{alertItem.submitted_by || 'Watchman'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Date</p>
                <p className="text-base font-semibold text-white">{alertItem.date ? new Date(alertItem.date).toLocaleDateString() : 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-700 pt-6">
            ...
            <div className="relative mb-4">
              <select
                ...
                className="w-full appearance-none rounded-lg border border-gray-700 bg-gray-800 px-4 py-3 text-sm text-white"
              >
              ...
              <ChevronDown className="pointer-events-none absolute right-3 top-3.5 text-gray-400" size={20} />
            </div>
            <div className="flex justify-end gap-4">
              ...
              <button
                ...
                className="px-6 py-2 rounded font-semibold bg-blue-600 hover:bg-blue-500 text-white disabled:cursor-not-allowed disabled:bg-gray-700 transition"
              >
                Assign Contractor
              </button>
            </div>
        ...
        <div className="space-y-6 px-6 py-5">
          <div>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
                {school.predictions.map((prediction, idx) => (
                  <div
                    key={`${school.school_id}-pred-${idx}`}
...
const DashboardDEO = () => {
      ...
      try {
        ...
        if (queueResponse?.ok) {
          const data = await queueResponse.json();
            const queueData = Array.isArray(data) ? data : (data.submissions || data.queue || []);
          ...
          if (approvedResponse?.ok) {
            ...
            const approvedData = Array.isArray(data) ? data : (data.submissions || []);
        }
  ...
  const handleAssign = async (contractorId) => {
    ...
    const payload = {
      submission_id: selectedSchool.submission_id || selectedSchool.id.toString(),
      contractor: contractor.name,
      priority_level: selectedSchool.priority_level || "Medium",
      notes: selectedSchool.condition || selectedSchool.issue || 'Maintenance required'
    };
          ...
          <div className="mt-4 space-y-3">
            {workOrdersLoading ? (
              ...
              workOrders.map((item, idx) => (
                <div key={`wo-${item.id}-${idx}`} className={`rounded-lg border ${isDark ? 'border-gray-800 bg-gray-950' : 'border-gray-200 bg-gray-50'} p-4`}>
                  <div className="flex items-center justify-between">
                    <div>
